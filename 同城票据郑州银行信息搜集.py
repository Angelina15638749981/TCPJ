...
# 创建文件夹
# 定义要创建的目录
mkpath = r"E:\同城票据_郑州_票面信息\%s" % nowTime
    ...
    try:
        ...
        for i in range(1, 16):
            with open('同城票据_接单中票面信息_%s.txt' % nowTime, 'a+') as f:
                ...
                driver = save_pic()
                driver.get(pic_link)
                driver.save_screenshot(r"E:\同城票据_郑州_票面信息\%s\%s_%s_%s_%s_%s_%s_%s.png" % (nowTime, time_pic_needed, item_person, item_amount, days_to_expire_date, interest_every_100_thousand,annual_interest,defect_spot))
